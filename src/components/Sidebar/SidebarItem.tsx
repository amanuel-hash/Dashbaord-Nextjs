...
const SidebarItem: React.FC<any> = ({ item, pathname }: any) => {
  ...
  return (
    <li>
      <Link
        ...
        className={`${
          isActive
            ? "bg-[#4caf50] text-[18px] font-bold text-white dark:bg-white/10 dark:text-white"
            : "text-black hover:bg-gray-2 hover:text-dark dark:text-gray-5 dark:hover:bg-white/10 dark:hover:text-white"
        } group relative flex items-center gap-3 rounded-[8px] px-3.5 py-3 duration-300 ease-in-out`}
      >
        {item.icon}
        {item.label}
